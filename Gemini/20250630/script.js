document.addEventListener('DOMContentLoaded', () => {
    ...
    submitBtn.addEventListener('click', () => {
        if (selectedNumbers.size === currentNumPicks) {
            ...
            // 1. 알림 확인 (여기서는 간단히 confirm 사용)
            const confirmMessage = `선택된 번호: ${sortedNumbers.join(', ')}\n이 번호로 응모하시겠습니까?`;
            if (confirm(confirmMessage)) {
                // 2. 선택된 번호를 다음 페이지로 전달하기 위해 localStorage에 저장
                // JSON.stringify를 사용하여 배열을 문자열로 변환하여 저장합니다.
                localStorage.setItem('userSelectedNumbers', JSON.stringify(sortedNumbers));
                localStorage.setItem('numberOfPicks', currentNumPicks); // 뽑은 개수도 저장

                // 3. 결과 페이지로 이동
                // 'result.html'은 같은 디렉토리에 있다고 가정합니다.
