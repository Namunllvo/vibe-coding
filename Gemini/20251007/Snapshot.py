...
def on_click(x, y, button, pressed):
    ...
    if pressed:
        ...
        if len(clicks) == 2:
            listener.stop()
            root.after(0, lambda: capture_area(clicks[0], clicks[1]))  # 메인 스레드에서 실행
...
def capture_area(pt1, pt2):
    ...
    messagebox.showinfo("완료", "캡처 완료! 저장하려면 '저장하기' 버튼을 눌러주세요.")
...
capture_btn = tk.Button(root, text="스크린샷 찍기",
                        command=lambda: threading.Thread(target=start_capture).start(),
                        width=20, height=2)
