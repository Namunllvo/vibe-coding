document.addEventListener('DOMContentLoaded', () => {
    ...
    const viewResultsBtn = document.getElementById('viewResultsBtn'); // 새로운 버튼 요소 가져오기
    ...
    submitBtn.addEventListener('click', () => {
        if (selectedNumbers.size === currentNumPicks) {
            ...
            if (confirm(confirmMessage)) {
                ...
                // 현재 선택된 번호를 새로운 응모 내역으로 추가
                const newEntry = {
                    id: Date.now(), // 고유 ID (타임스탬프)
                    numbers: sortedNumbers,
                    maxRange: currentMaxNumber, // 응모 당시의 번호 범위
                    numPicks: currentNumPicks,   // 응모 당시의 뽑은 개수
                    date: new Date().toLocaleString() // 응모 날짜/시간
                };
                lotteryEntries.push(newEntry);

                // 업데이트된 응모 내역을 localStorage에 저장
                saveLotteryEntries(lotteryEntries);

                // 선택된 번호 초기화 (다음 응모를 위해)
                selectedNumbers.clear();
                updateUI(); // UI 초기화 반영
